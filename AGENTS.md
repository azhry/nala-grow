...
- Local browser E2E must be visible/headed by default so the human can watch. Use headless only in CI, when the user explicitly asks, or for a final artifact-only check; if visible mode is unavailable, stop and report it.
