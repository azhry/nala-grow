...
- Mark work complete only when the PR is merged or the user/team workflow confirms completion. For an intentional red-test issue, use the paired delivery workflow below to distinguish its expected red command from the required green merge checks.
...
3. Keep the intentional red command isolated in the explicitly documented suite, build tag, or test lane. Do not skip, quarantine, weaken, or convert the assertion into a false pass. Ordinary required build, lint, unit, and unaffected integration checks must remain green.
4. Do not mark the test issue blocked merely because the explicitly expected assertion is red. That is the required deliverable. If the red command is itself configured as a required GitHub check, do not bypass branch protection; record the CI-policy blocker until the repository has an approved intentional-red lane.
5. Record the exact test commit SHA, branch, PR, focused command, nonzero exit, intended assertion failure, successful setup stages, isolation mechanism, and required green checks in both the test issue and paired implementation issue.
6. When the issue contract explicitly authorizes an intentional-red merge and every required GitHub check is green, merge the test-only PR into `main`. The test issue then completes through its own merged PR.
