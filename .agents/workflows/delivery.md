...
- Before PR creation or tracker transition, verify: applicable PR template read, saved PR body re-read, `git status --short`, `git diff --cached --name-status`, and `gh pr view` confirms URL, draft state, base/head, mergeability, and checks.
- Before PR creation or tracker transition, validate each PR body against the applicable template: include the direct PR URL in `Review and merge order`, link every predecessor and successor in the stated order, keep automated checks separate from manual acceptance evidence, use Bash for manual command blocks by default, and remove every placeholder, fake record, mock endpoint, and unrun flow presented as a pass.
...
- For API, authentication, persistence, migration, or infrastructure changes, do not declare the live boundary blocked until the documented project knowledge and any available read-only `nala-infra` path have been checked. Record whether the blocker is dependency access, application deployment, or an unverified fixture.
...
- Preserve an existing ready-for-review state; do not convert it back to draft unless the user explicitly asks.
