...
export default function DashboardPage() {
  ...
  return <div className="pb-stack-lg"><AppHeader /><div className="mx-auto max-w-6xl space-y-stack-md px-container-margin pt-16 md:px-stack-lg">
    ...
    {loadState === "loading" && <p role="status" className="rounded-2xl bg-surface-container-low p-gutter text-body-sm text-on-surface-variant">Loading dashboard records…</p>}
     {loadState === "error" && <div role="alert" className="flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-surface-container-low p-gutter text-body-sm text-on-surface-variant"><span>We couldn&apos;t load your dashboard right now.</span><Link href="/profile/create" className="inline-flex items-center gap-1 rounded-full bg-primary px-4 py-2 font-label-md text-label-md text-on-primary shadow-sm transition-transform active:scale-[0.98]"><span aria-hidden="true" className="material-symbols-outlined text-[18px]">add</span>Create a profile</Link></div>}
     <section aria-label="Quick actions" className="relative"><div className="flex gap-stack-sm overflow-x-auto pb-2 hide-scrollbar">{quickActions.map((action) => <button key={action.label} type="button" onClick={() => setActiveSection((current) => current === action.section ? null : action.section)} aria-pressed={activeSection === action.section} className={["flex items-center gap-base whitespace-nowrap rounded-full px-gutter py-stack-sm text-label-md font-label-md shadow-sm transition-all active:scale-95 hover:shadow-md", activeSection === action.section ? "bg-primary text-on-primary" : "border border-primary/20 bg-surface-container-highest text-primary"].join(" ")}><span aria-hidden="true" className="material-symbols-outlined">{action.icon}</span>{action.label}</button>)}</div><p className="text-body-sm text-on-surface-variant md:hidden">Swipe for more actions</p></section>
